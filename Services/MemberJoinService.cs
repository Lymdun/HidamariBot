...
public class MemberJoinService : DiscordBotService {
    ...
    const string GUILD_NAME = "50 Nuances de Kikoojaps";
    ...
    async Task SendJoinNotificationAsync(Snowflake guildId, IMember newMember) {
        try {
            var message = new LocalMessage()
                .WithContent(
                    $"{Mention.User(newMember.Id)} Bienvenue sur {GUILD_NAME}, pour passer la frontière il faut remplir le questionnaire {QUESTIONNAIRE_MESSAGE_PATH} 📋");
